...
- **休館期間的線是斷開的，不是壓成 0。** 讀數留在軸上（格子就是防止 35 天塌成一步的東西），但不畫過去。壓成 0 更糟：**0 是真的會出現的讀數**——多數日子開館時就是 0，端午節整天都是 0——所以那會讓「關著」和「開著但沒人」在圖上長得一樣。斷線加上黃底區間說得出「關了」而不宣稱一個沒人量到的數字。tooltip 對空值回 `closed`，不是 `null people`。`connectNulls` 絕對不能設，設了整件事就白做，`test_the_line_is_not_reconnected_across_the_gap` 盯著它。
